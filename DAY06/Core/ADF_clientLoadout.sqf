...
if ((_ADF_unitFaction == "BLU_F") && _ADF_customLoadout_MOD) exitWith { // BLUFOR: Custom loadout = true
	...
	if (
		...
		(toString _ADF_unitCheck != "fod") &&
		(toString _ADF_unitCheck != "foc")
	) exitWith {
	...
	if (_s == "fod") exitWith {[_s,_r] call ADF_fnc_loadoutFod}; // FOB INF DAGUET
	if (_s == "foc") exitWith {[_s,_r] call ADF_fnc_loadoutFoc}; // FOB INF CE
